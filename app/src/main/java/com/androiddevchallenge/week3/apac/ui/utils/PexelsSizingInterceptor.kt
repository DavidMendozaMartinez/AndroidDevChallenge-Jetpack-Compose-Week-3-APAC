...
/**
 * A Coil [Interceptor] which appends query params to Pexels urls to request sized images.
 */
@OptIn(ExperimentalCoilApi::class)
object PexelsSizingInterceptor : Interceptor {
    override suspend fun intercept(chain: Interceptor.Chain): ImageResult {
        val data = chain.request.data
        val size = chain.size
        if (data is String &&
            data.startsWith("https://images.pexels.com/photos/") &&
            size is PixelSize
        ) {
